...
def _safe_header_index(headers, name, fallback=-1):
    """
    headers listesinde name'i arar. Bulamazsa fallback döner.
    compute_stats() içinde ValueError riskini ortadan kaldırır.
    """
    try:
...
def compute_stats(headers, rows):
    ...
    idx_sarki   = _safe_header_index(headers, "Şarkı Adı")
    idx_sanatci = _safe_header_index(headers, "Sanatçı")
    idx_sure    = _safe_header_index(headers, "Süre (ms)")
    idx_tarih   = _safe_header_index(headers, "Dinlenme Tarihi")
    idx_iso     = _safe_header_index(headers, "_played_at_iso")
    idx_album   = next(
        (i for i, h in enumerate(headers) if (h or "").strip() in ("Albüm", "Album", "albüm", "album")),
        -1,
    )

    # Zorunlu sütunlar bulunamazsa None dön
    if idx_sarki == -1 or idx_sanatci == -1 or idx_sure == -1 or idx_tarih == -1:
        logger.warning(f"compute_stats: Zorunlu sütun eksik. Mevcut: {headers}")
        return None
    ...
    for row in rows:
        min_col = max(idx_sarki, idx_sanatci, idx_sure, idx_tarih)
        if len(row) <= min_col:
            continue
    ...
    for i in range(6, -1, -1):
        ...
        hafta.append({
            "tarih":   ts,
            "gun":     config.TR_GUNLER[gun.weekday()],
            "sure_sn": gun_sure.get(ts, 0),
        })
