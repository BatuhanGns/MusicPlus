...
# H sütunu (index 7) gizli duplicate kontrol sütunu — ISO timestamp
HAM_HEADERS = [
...
class SheetsClient:
    ...
    def _ensure_sheets(self):
        ...
        if not ham_ws:
            ...
            if first_row != HAM_HEADERS:
                # Başlık yanlışsa 1. satırı güncelle
    ...
    def _get_existing_played_ats(self):
        """H sütunundaki ISO timestamp'leri oku — duplicate kontrolü için"""
        ws = self._find_sheet("Ham")
        # H sütunu = index 8
        col_values = ws.col_values(8)
        return set(col_values[1:])  # başlığı atla
    ...
    def append_ham(self, tracks: list) -> int:
        ...
        for t in tracks:
            ...
            if iso not in existing:
                new_rows.append([
                    format_tarih(iso),   # A: DD.MM.YYYY (görünen)
                    t["track_id"],        # B
                    t["track_name"],      # C
                    t["artist_name"],     # D
                    t["album_name"],      # E
                    t["duration_ms"],     # F
                    t["duration_sec"],    # G
                    iso,                  # H: ISO timestamp (duplicate kontrol)
                ])
                existing.add(iso)
