...
class SheetsClient:
    ...
    def append_ham(self, tracks: list) -> int:
        ...
        if new_rows:
            # İkisine de yaz
            ham_ws.append_rows(new_rows, value_input_option="RAW")
    ...
    def _trim_ham_to_1000(self, ws):
        """Ham sayfasını 1000 kayıtta tut, fazlasını sil"""
        all_rows = ws.get_all_values()
        data_rows = len(all_rows) - 1
        if data_rows > 1000:
            fazla = data_rows - 1000
            ws.delete_rows(2, fazla + 1)
            logger.info(f"🗑️ {fazla} eski kayıt Ham'dan silindi.")
